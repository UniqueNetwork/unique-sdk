...
export function createConfig(options) {
  const { packageName, entryPoints, mainPackageJson, currentPackageJson } =
    options;

  const SRC_FOLDER = `./packages/${packageName}`;
  const TS_CONFIG = 'tsconfig.lib.json';
  const DIST_FOLDER = `./dist/packages/${packageName}`;
  ...
  const INTERNAL_IDS = entryPoints.map((e) => path.join(SRC_FOLDER, e));
  ...
  const allConfigs = entryPoints.reduce(
