...
import { SubstrateModule, SubstratePrimaryModule } from '../modules/substrate/substrate.module';
import { Config } from '../config/config.module';

export const addSwagger = (app: INestApplication) => {
  const configService: ConfigService<Config> = app.get(ConfigService);
  const { chainWsUrl, name } = configService.get<Config['secondary']>('secondary');
  const secondaryDescription = (chainWsUrl && name) ? [
    `Instance have secondary substrate connection with prefix ${name}`,
    `please see swagger at <a href="./${name}">${configService.get('swagger')}/${name}</a>`,
  ].join('\n\n') : null;
  ...
  const uniqueDocument = SwaggerModule.createDocument(
    app,
    config,
    {
